/*
Naive Bayes:
Implement a Naive Bayes classifier by finalizing the two given classes. 
The BayesianLearner class acts like a builder for the BayesianClassifier instances. 
That means that the learner gets the set of classes during its creation and the learnExample method of the learner is called once for each
document of the training set. Internally, the learner should gather all statistics that are necessary for the classifier 
when processing the training examples. After the learner saw all training documents, the createClassifier method is called which creates an instance
of the BayesianClassifier class and initializes it with the statistics gathered before. 
...
public class BayesianClassifier {
    ...
    public BayesianClassifier(){
        stopWordsList = new ArrayList<String>(Arrays.asList(stopWords));
    }
    /**
    ...
    public String testModel(String text) {
...
//         stopWordsList = new ArrayList<String>(Arrays.asList(stopWords));
    ...
    public void learnExample(String clazz, String text) {
        ...
        if(classes_hashmap.containsKey(clazz)) {
            class_counter = classes_hashmap.get(clazz);
        }
        class_counter ++;
        classes_hashmap.put(clazz, class_counter);
        ...
        for (int i = 0; i < splited_text.length; i++) {
            ...
            if (! isStopWord(splited_text[i])) {
                ...
                // get the class
                if(tokens_count_in_class.containsKey(clazz)) {
                    vocab_count = add_vocab_count(clazz, splited_text[i]);
                }else {
                    tokens_count_in_class.put(clazz, vocab_count);
                    vocab_count = add_vocab_count(clazz, splited_text[i]);
                }
                tokens_count_in_class.put(clazz, vocab_count);
            }
